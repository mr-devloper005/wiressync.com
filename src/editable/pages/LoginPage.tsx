    ...
    <EditableSiteShell>
      <main className="editable-page-enter bg-[#f7f4ef] text-[#111]">
        <section className="mx-auto grid min-h-[calc(100vh-12rem)] max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[1.08fr_0.92fr]">
          <div className="flex flex-col justify-center border-b border-black bg-[#2f80ed] p-8 text-white sm:p-12 lg:border-b-0 lg:border-r lg:p-16">
            ...
            <div className="editable-stagger mt-8 grid gap-3 text-sm font-black uppercase tracking-[0.12em] text-white/75">
              <span className="border-t border-white/30 pt-3">Create release drafts</span>
              <span className="border-t border-white/30 pt-3">Access submission tools</span>
              <span className="border-t border-white/30 pt-3">Return to the media desk</span>
            </div>
          </div>
          <div className="flex flex-col justify-center p-7 sm:p-12 lg:p-16">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[#2f80ed]">Member access</p>
            <h2 className="editorial-serif mt-3 text-4xl font-black">{pagesContent.auth.login.formTitle}</h2>
            <EditableLocalLoginForm />
            <p className="mt-5 border-t border-black pt-5 text-sm text-black/65">New here? <Link href="/signup" className="font-black text-[#2f80ed] underline-offset-4 hover:underline">{pagesContent.auth.login.createCta}</Link></p>
          </div>
        </section>
      </main>
    </EditableSiteShell>

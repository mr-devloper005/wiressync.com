  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 border-t-4 border-black bg-white pt-7">
      ...
      <label className="mt-4 grid gap-2 text-sm font-black opacity-75">
        Message
        <textarea name="message" required rows={6} placeholder="Tell us what you need help with..." className="border border-black bg-white px-4 py-3 text-base font-medium outline-none transition focus:border-[#2f80ed]" />
      </label>
      ...
      <button type="submit" disabled={status === 'submitting'} className="mt-6 inline-flex h-[3.25rem] w-full items-center justify-center gap-2 border border-black bg-black px-6 text-xs font-black uppercase tracking-[0.24em] text-white transition hover:bg-[#2f80ed] disabled:cursor-not-allowed disabled:opacity-70">
...
function Field({ name, label, type = 'text', placeholder, required = false }: { name: string; label: string; type?: string; placeholder?: string; required?: boolean }) {
  return (
    <label className="grid gap-2 text-sm font-black opacity-75">
      {label}
      <input name={name} type={type} required={required} placeholder={placeholder} className="h-[3.25rem] border border-black bg-white px-4 text-base font-medium outline-none transition focus:border-[#2f80ed]" />
    </label>
  )
}
